...
export default function SplashCard({ title, subTitle, description, icon, link, linkTitle, typecard }: { title: string, subTitle: string, description: string, icon: {
  ...
  return (
    <Link to={link}>
      <span className={`profile-card ${typecard} c1`}>
        <div className="card-corner"></div>
        <div className="card-icon">
          <svg viewBox={icon.viewBox} fill={icon.fill} stroke={icon.stroke} strokeWidth={icon.strokeWidth}>
            <path d={icon.d}></path>
          </svg>
        </div>
        ...
        <div className="card-arrow">
          {linkTitle}
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
        </div>
      </span>
    </Link>
  )
